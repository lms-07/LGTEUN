...
# based on https://github.com/yggame/MDCUN/blob/main/model/pan_unfolding_v4.py
...
class pan_unfolding(nn.Module):
    def __init__(self, cfg, logger, mid_channels=64, T=4):
        super(pan_unfolding, self).__init__()
        ...
        self.conv_u = nn.ModuleList([nn.Sequential(*[
            nn.Conv2d(self.in_channels * (i + 1), 64, kSize, padding=(kSize - 1) // 2, stride=1),
            nn.Conv2d(64, self.in_channels, kSize, padding=(kSize - 1) // 2, stride=1)
        ]) for i in range(T)])
        ...
        self.conv_up = Conv_up(self.in_channels, G0, self.up_factor)
        self.conv_down = Conv_down(self.in_channels, G0, self.up_factor)

        self.rm1 = att_spatial(res_num=3)

        self.NLBlock = blockNL(self.in_channels, 15)

        self.hf_pan = nn.Conv2d(3, 1, 1, padding=0, stride=1)

        self.conv1x1 = nn.Conv2d(4, self.in_channels, 1)
    ...
    def forward(self, lms, pan):
        ...
        for i in range(len(self.conv_u)):
            ...
            if self.in_channels > 4:
                decode_u = self.conv1x1(decode_u)
            ...
            decode_v = torch.cat([rm2_s2_0, rm2_s2_1, rm2_s2_2, rm2_s2_3], 1)

            if self.in_channels > 4:
                decode_v = self.conv1x1(decode_v)  # B 4 256 256
...
class MDCUN(Base_model):
    def __init__(self, cfg, logger, train_data_loader, test_data_loader0, test_data_loader1):
        ...
        self.add_module('core_module', pan_unfolding(cfg=cfg, logger=logger, **G_cfg))
